...
//	container by scanning for JPEG SOI markers (0xFF 0xD8 0xFF) within the
//	mdat box. Falls back to full-file hash if extraction fails.
//
// Magic bytes:
//
...
// HashableReader returns an io.ReadCloser over the embedded JPEG preview
// extracted from the CR3 ISOBMFF container. Falls back to the full file.
...
// the embedded full-resolution JPEG preview in a CR3 file.
//
// CR3 stores a JPEG preview in the mdat box. We scan for the largest
// JPEG blob (SOI marker 0xFF 0xD8 0xFF) within the file.
func findCR3JpegPreview(r io.ReadSeeker) (*jpegPreview, error) {
	...
	for _, box := range topBoxes {
		...
		// Scan the mdat box for JPEG SOI markers.
		dataStart := box.dataOff
		dataSize := box.size - (box.dataOff - box.offset)
		if dataSize <= 0 || dataSize > 512*1024*1024 { // sanity: max 512 MB
			continue
		}

		// Read the mdat content.
		if _, err := r.Seek(dataStart, io.SeekStart); err != nil {
			continue
		}
		mdatData := make([]byte, dataSize)
		n, err := io.ReadFull(r, mdatData)
		if err != nil && n == 0 {
			continue
		}
		mdatData = mdatData[:n]

		// Find all JPEG blobs within mdat.
		preview := findLargestJPEGInData(mdatData, dataStart)
...
// findLargestJPEGInData scans data for JPEG SOI markers and returns the
// largest JPEG blob found, with its file offset.
func findLargestJPEGInData(data []byte, baseOffset int64) *jpegPreview {
	var largest *jpegPreview
	...
	for i := 0; i+3 < len(data); i++ {
		...
		// Found a potential JPEG start. Find the EOI (0xFF 0xD9).
...
// findJPEGEnd scans forward from start to find the JPEG EOI marker (0xFF 0xD9).
// Returns the index just past the EOI, or 0 if not found.
