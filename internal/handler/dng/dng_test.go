...
func TestHandler_Extensions(t *testing.T) {
	h := New()
	exts := h.Extensions()
	if len(exts) != 1 || exts[0] != ".dng" {
		t.Fatalf("Expected [.dng], got %v", exts)
	}
}

// TestHandler_MagicBytes verifies the correct magic signatures are returned.
func TestHandler_MagicBytes(t *testing.T) {
	h := New()
	sigs := h.MagicBytes()
	if len(sigs) != 2 {
		t.Fatalf("Expected 2 magic signatures, got %d", len(sigs))
	}
	// Check LE signature
	if sigs[0].Offset != 0 || !bytes.Equal(sigs[0].Bytes, []byte{0x49, 0x49, 0x2A, 0x00}) {
		t.Fatalf("LE signature mismatch: %v", sigs[0])
	}
	// Check BE signature
	if sigs[1].Offset != 0 || !bytes.Equal(sigs[1].Bytes, []byte{0x4D, 0x4D, 0x00, 0x2A}) {
		t.Fatalf("BE signature mismatch: %v", sigs[1])
	}
}
...
func TestHandler_Detect_valid(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeDNG(t, dir, "test.dng")
	...
	if !ok {
		t.Fatal("Detect should return true for valid DNG")
	}
}
...
func TestHandler_Detect_wrongExtension(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeDNG(t, dir, "test.jpg")
...
func TestHandler_Detect_wrongMagic(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.dng")
...
// TestHandler_MetadataSupport verifies that the DNG handler declares MetadataSidecar.
...
func TestHandler_WriteMetadataTags_noop(t *testing.T) {
	...
	err := h.WriteMetadataTags("dummy.dng", tags)
	if err != nil {
