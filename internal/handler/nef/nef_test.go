...
func TestHandler_Extensions(t *testing.T) {
	h := New()
	exts := h.Extensions()
	if len(exts) != 1 || exts[0] != ".nef" {
		t.Fatalf("Expected [.nef], got %v", exts)
	}
}
...
func TestHandler_Detect_valid(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeNEF(t, dir, "test.nef")
	...
	if !ok {
		t.Fatal("Detect should return true for valid NEF")
	}
}
...
func TestHandler_Detect_wrongExtension(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeNEF(t, dir, "test.jpg")
...
func TestHandler_Detect_wrongMagic(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.nef")
...
// TestHandler_MetadataSupport verifies that the NEF handler declares MetadataSidecar.
...
func TestHandler_WriteMetadataTags_noop(t *testing.T) {
	...
	err := h.WriteMetadataTags("dummy.nef", tags)
	if err != nil {
