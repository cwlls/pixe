...
// Handler implements domain.FileTypeHandler for Pentax PEF RAW images.
type Handler struct {
	tiffraw.Base
}

// New returns a new PEF Handler.
...
// Pentax PEF files use little-endian byte order.
...
// Detect returns true if the file has a .pef extension AND begins with
...
func (h *Handler) Detect(filePath string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("pef: open %q: %w", filePath, err)
	}
