...
func TestHandler_Extensions(t *testing.T) {
	h := New()
	exts := h.Extensions()
	if len(exts) != 1 || exts[0] != ".arw" {
		t.Fatalf("Expected [.arw], got %v", exts)
	}
}
...
func TestHandler_Detect_valid(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeARW(t, dir, "test.arw")
	...
	if !ok {
		t.Fatal("Detect should return true for valid ARW")
	}
}
...
func TestHandler_Detect_wrongExtension(t *testing.T) {
	dir := t.TempDir()
	filePath := buildFakeARW(t, dir, "test.jpg")
...
func TestHandler_Detect_wrongMagic(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.arw")
...
// TestHandler_MetadataSupport verifies that the ARW handler declares MetadataSidecar.
...
func TestHandler_WriteMetadataTags_noop(t *testing.T) {
	...
	err := h.WriteMetadataTags("dummy.arw", tags)
	if err != nil {
