...
`IMG_0001.jpg` from your phone and `IMG_0001.jpg` from your partner's phone are correctly identified as different files, even though they have the same name. The same photo renamed to `vacation_sunset.jpg` is correctly identified as a duplicate of the original, even though the name is different. A RAW file whose embedded JPEG preview was regenerated by Lightroom is still correctly identified as the same file, because the sensor data — the ground truth — hasn't changed.
